...
function parseTransactionDate(value) {
  ...
  return new Date(Date.UTC(Number(digits.slice(0, 4)), Number(digits.slice(4, 6)) - 1, Number(digits.slice(6, 8)), Number(digits.slice(8, 10)), Number(digits.slice(10, 12)), Number(digits.slice(12, 14))))
}
...
export function extractMpesaReceipt(callbackMetadata = []) {
  const values = Object.fromEntries(callbackMetadata.filter((entry) => entry?.Name).map((entry) => [entry.Name, entry.Value]))
  return {
