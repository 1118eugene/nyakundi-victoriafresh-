...
export default function Login() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await requestLoginOtp({ email: email.trim() || undefined, phone: email.trim() ? undefined : phone.trim() || undefined })
  ...
  async function handleResend() {
    ...
    try {
      ...
      setResendIn(60)
    } catch (err) { setError(err instanceof Error ? err.message : 'Unable to resend your sign-in code.') }
    finally { setSubmitting(false) }
  }
  ...
  return (
    <section className="auth-page"><div className="auth-shell auth-shell-compact">
      ...
      <div className="auth-form-panel"><div className="auth-form-heading"><span className="auth-step">SECURE CUSTOMER SIGN-IN</span><h2>Sign in to continue</h2><p>Enter your registered email or phone. We&apos;ll send a one-time code to your verified phone.</p></div>
        ...
        {otpSent && <div className="otp-panel"><div><span className="auth-step">VERIFICATION REQUIRED</span><h3>Check your phone</h3><p>Enter the one-time code sent to <strong>{phone}</strong>.</p></div><label>Verification code<input value={otp} onChange={e => setOtp(e.target.value)} placeholder="000000" inputMode="numeric" maxLength={6} autoComplete="one-time-code" /></label><button type="button" onClick={() => void handleVerify()} disabled={submitting} className="auth-submit">Verify and enter the shop</button><button type="button" onClick={() => void handleResend()} disabled={submitting || resendIn > 0} className="auth-secondary">{resendIn ? `Resend code in ${resendIn}s` : 'Resend code'}</button><p className="auth-help">You can edit your email or phone above before requesting another code.</p></div>}
